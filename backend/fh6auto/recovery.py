...
class RecoveryService:
    ...
    def restart_game_and_boot(self, force_test=False):
        ...
        if not force_test:
            if not self.config.values.get("auto_restart", False):
                self.log("未开启自动重启，任务结束。")
                return False

        self.log("触发启动机制！正在拉起游戏...")
        try:
            cmd_str = self.config.values.get("restart_cmd", "start steam://run/2483190")
            os.system(cmd_str)
        ...
        for _ in range(120):
            ...
            if not self.state.is_running:
                return False
            if self.game_window.check_and_focus_game():
                process_found = True
                break
            time.sleep(1)
        ...
        while self.state.is_running and time.time() - start_time < 300:
            self.check_pause()
            ...
            if not passed_screen_1:
                pos_h6 = self.image_matcher.find_image_sift(
                    "horizon6.png",
                    min_inliers=8,
                )
            ...
            if passed_screen_1:
                pos_continue = self.text_detector.find_any_text_ui(
                    ["继续"],
                )
                if pos_continue:
                    self.log("识别到 画面2 (继续按钮)，进行点击...")
                    self.input_actions.game_click(pos_continue)
    ...
    def check_vramne_during_race(self):
        try:
            pos_vram = self.image_matcher.find_image_sift(
                "VRAMNE.png",
                min_inliers=24,
            )
            if pos_vram:
    ...
    def enter_menu(self):
        ...
        for i in range(60):
            ...
            pos_menu = self.image_matcher.find_image_sift(
